...
from .CoordConv import get_coord_maps
...
class ROMP(Base):
    ...
    def head_forward(self,x):
        ...
        x = torch.cat((x, self.coordmaps.to(x.device).repeat(x.shape[0],1,1,1)), 1) # [B, 2, 128, 128]

        params_maps = self.final_layers[1](x)   # [B, 23*6, 128, 128]
        center_maps = self.final_layers[2](x)   # [B, 1, 128, 128]
